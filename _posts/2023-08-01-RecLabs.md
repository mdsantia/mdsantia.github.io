...
Setting two constant rates (of Infections _a(SI)_ & of Recovered _b(I)_) we can see, given that the subpopulations are mutually exclusive, that the number of susceptible people decreases only if the number of infections increases and the oppostite relationship occurs between infected and recovered. Meaning,
