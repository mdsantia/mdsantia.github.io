...
## Introduction
It is incredibly crucial to understand the two theorems foundational to our discussion today:
...
1. 
![x'=\frac{1}{t}](https://latex.codecogs.com/svg.image?x'=\frac{1}{t})
and any 
...
2.
![x'=\sqrt(x)=x^(1/2)](https://latex.codecogs.com/svg.image?x'=\sqrt{x}=x^{\frac{1}{2}})
and any 
![x_0=0](https://latex.codecogs.com/svg.image?x_0=0).
...
Note that their is an intentional word choice to describe the theorems and whether or not not it *holds*. This is because the converse of each theorem is necessarily not true (I am not sure if the converse of Uniqueness as I could not find or think of a counterexample but definitely Existence). An easy example is covered in __Problem 2__ with
![x'=A\frac{x}{t}](https://latex.codecogs.com/svg.image?x'=A\frac{x}{t})
where Existence does not hold (and by consequence Uniqueness as well) for any *t=0*.
...
## Problem 2
First we need to define the differential equation we are going to work on. We also need to define the equations resulting when solving the right hand side of the equation with the proper input of initial values.
```Maple
#2a
...
f(x)
```
and it outputs the explicit value of the solution to the system.
