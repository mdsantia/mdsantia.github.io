...
## Introduction
After some theory discussion, **Lab 5** will dive into a deeper mathematical model that can help us build predictions based on data. We will learn about three different prediction models of population growth: 
...
We will also try to objectively determine which model is a best representation of the data. The most important detail to know of the models, is that we are going to treat *t* to represent the number of decades after 1790, making our initial data point *t=0* to be that same year.
...
### Problem 6
Hopefully, from the previous plot, it is self-evident which model best represents the data. However, to objectively determine the most accurate model we will need to utilize a metric, in this case the norm, and average over the number of data points. The biggest value would then imply the biggest difference form the data.
...
are the populations after *h* decade steps of the initial population chosen, 
