...
## Introduction
This lab is very unusual to the other labs throughout the semester. Maple, is an incredibly powerful tool to process algebraic analysis without the need of manual arithmetic manipulation. The main things to understand of this lab is the syntax and main functions shown in the discussion section of the handout.

## Main Syntax and Functions
1. To generate a variable, just name it as usual, and utilize the 
` Maple
:=
`
...
``` Maple
equation1 := diff(exp(2*x),x) + x^2*y
```
where 
` Maple
exp(x)
...
# x_0 and y_0 are our initial conditions, and we need to specify the structure of the function by it dependent and independent variable
```
